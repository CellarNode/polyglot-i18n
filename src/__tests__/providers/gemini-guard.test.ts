...
const { hashValue } = await import("../../cache.js");
...
describe("byte-identical output reaches the cache decision", () => {
  it("writes a byte-identical correct value, warns, and does NOT cache it", async () => {
    ...
    expect(result.staleSourceKeys).toEqual(["brand.retailer"]);
  });
  ...
  it("keeps the previous translation over the identical value when there is one", async () => {
    ...
    const result = await translateNamespace({
      ...
      cacheEntries: {},
      // What the per-language cache adds: zh evicted this key at this exact
      // English, so the Chinese on disk is known to answer it.
      retriedSourceKeys: new Set(["nav.overview"]),
      provider: provider(),
      targetLang: "zh",
      force: false,
    });
    ...
    expect(result.sourceProvenance["nav.overview"]).toBe(
      hashValue("Producer dashboard")
    );
  });
  ...
  it("still evicts when the previous translation IS the English source", async () => {
    ...
    // the CEL-1539 hole in one line. Same inputs, same eviction on the record —
    // only the value on disk differs.
    respondWith(JSON.stringify({ "nav.overview": "Producer dashboard" }));
  ...
  it("mints no accept under --force, whatever the eviction record says", async () => {
    // CEL-1543 P1: `--force` is the recovery command, so it always evicts.
    respondWith(JSON.stringify({ "nav.overview": "Producer dashboard" }));
...
describe("GeminiProvider.translate — a corroborated uniform echo", () => {
  ...
  it("writes no English into the fallback categories", async () => {
    ...
    expect(result.staleSourceKeys.sort()).toEqual([
      "download.labels_one",
      "download.labels_other",
    ]);
  });
...
describe("a correct-by-necessity uniform group costs one round, not one per run", () => {
  ...
  it("retries and evicts on the first encounter", async () => {
    ...
    expect(result.staleSourceKeys).toEqual(["volume.ml_other"]);
    // And the eviction carries no provenance, because the Russian on disk came
    // from before this cache had ever heard of the key.
    expect("volume.ml_other" in result.sourceProvenance).toBe(false);
  });
  ...
  it("stops paying once the eviction it wrote is on the record", async () => {
    ...
    expect(result.staleSourceKeys).toEqual([]);
    expect(result.sourceProvenance["volume.ml_other"]).toBe(
      hashValue("{{count}} ml")
    );
  });
